...
import com.deguru.submission.config.KafkaSseConsumer;
import com.deguru.submission.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class SubmissionTrackerAngKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/submission-tracker-ang-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/submission-tracker-ang-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/submission-tracker-ang-kafka/unregister"));
                return;
            }
        }
